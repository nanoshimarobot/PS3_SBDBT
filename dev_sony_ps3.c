...
{
	BYTE	button_state1 = 0x00;
    BYTE    button_state2 = 0x00;
    
    sixaxis_button_state1 = button_state1;
    sixaxis_button_state2 = button_state2;
    ...
    if(hid_report[2]&0x10){
        button_state1 |= PS3_OUT_DIGITAL_UP;
    }
    if(hid_report[2]&0x80){
        button_state1 |= PS3_OUT_DIGITAL_LEFT;
    }
    if(hid_report[2]&0x40){
        button_state1 |= PS3_OUT_DIGITAL_DOWN;
    }
    if(hid_report[2]&0x20){
        button_state1 |= PS3_OUT_DIGITAL_RIGHT; 
    }
    if(hid_report[3]&0x04){
        button_state1 |= PS3_OUT_DIGITAL_L1;
    }
    if(hid_report[2]&0x01){
        button_state1 |= PS3_OUT_DIGITAL_SELECT;
    }
    if(hid_report[2]&0x02){
        button_state1 |= PS3_OUT_DIGITAL_L3;
    }

    
    if(hid_report[3]&0x10){
        button_state2 |= PS3_OUT_DIGITAL_TRIANGLE;
    }
    if(hid_report[3]&0x80){
        button_state2 |= PS3_OUT_DIGITAL_SQUARE;
    }
    if(hid_report[3]&0x40){
        button_state2 |= PS3_OUT_DIGITAL_CROSS;
    }
    if(hid_report[3]&0x20){
        button_state2 |= PS3_OUT_DIGITAL_CIRCLE;
    }
    if(hid_report[3]&0x08){
        button_state2 |= PS3_OUT_DIGITAL_R1;
    }
    if(hid_report[2]&0x08){
        button_state2 |= PS3_OUT_DIGITAL_START;
    }
    if(hid_report[2]&0x04){
        button_state2 |= PS3_OUT_DIGITAL_R3;
    }
    sixaxis_button_state1 |= button_state1;
    sixaxis_button_state2 |= button_state2;
	//analog state
    ...
    sixaxis_l2 = hid_report[18];//l2
    sixaxis_r2 = hid_report[19];//r2
	out_button_state();
	return;
}
