...
//#if FORMAT_RCB3
	...
	//analog val
	lineBuffer[pos++]=PS3_stick_limit(sixaxis_lx);			//4バイト目:左スティックX状態(0x00-0x7f) アナログのようでデジタル
	lineBuffer[pos++]=-PS3_stick_limit(sixaxis_ly);			//5バイト目:左スティックY状態(0x00-0x7f) アナログのようでデジタル
//    lineBuffer[pos++]=sixaxis_l2;
//    lineBuffer[pos++]=sixaxis_r2;
	lineBuffer[pos++]=PS3_stick_limit(sixaxis_rx);			//6バイト目:右スティックX状態(0x00-0x7f) アナログのようでデジタル
	lineBuffer[pos++]=-PS3_stick_limit(sixaxis_ry);			//7バイト目:右スティックY状態(0x00-0x7f) アナログのようでデジタル
